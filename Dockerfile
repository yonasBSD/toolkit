...
RUN cargo install --git https://github.com/BLAKE3-team/BLAKE3 b3sum && mv /root/.cargo/bin/b3sum /usr/local/bin
...
# Run dra installs
RUN dra download --automatic ovh/venom && mv venom.linux-amd64 /usr/local/bin/venom
RUN dra download --automatic BLAKE3-team/BLAKE3 && mv b3sum_linux_x64_bin /usr/local/bin/b3sum
